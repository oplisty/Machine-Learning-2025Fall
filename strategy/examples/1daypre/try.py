...
XGB_OUT_DIR = os.path.join(BASE_DIR, "ml_model", "output", "xgboost","horizon_1")
...
def load_xgb_results_and_features():
    ...
    if not (os.path.exists(train_path) and os.path.exists(valid_path) and os.path.exists(test_path)):
        raise FileNotFoundError("找不到 XGBoost train/valid/test 结果，请检查 ml_model/output/xgboost/horizon_1 目录。")
...
class ConvictionFilterStrategy(BaseMLStrategy):
    ...
    params = dict(
        ...
        w_pred=0.42,
        w_alpha=0.58,
        q_exit=0.09,   # bottom 9% 才空仓
        q_half=0.12,   # bottom 9~12% 半仓
    )
