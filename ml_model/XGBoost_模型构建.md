...
1. **数据加载**：从 CSV 文件加载原始 K 线数据，包含时间戳（`timestamps`）和六个特征列（`open`, `high`, `low`, `close`, `volume`, `amount`）。

2. **时间排序**：按照时间戳对数据进行升序排序，确保时间序列的连续性。

3. **时间范围过滤**：根据配置的 `fit_start_time` 和 `fit_end_time` 参数，过滤出用于特征拟合的数据范围。为确保测试集数据完整性，系统会自动检查并扩展 `fit_end_time` 至测试集结束时间。

### 3.3.2 时间序列数据集划分

采用**时间序列划分策略**（Time Series Split），按照时间顺序将数据划分为训练集、验证集和测试集：

- **训练集**：2018-01-02 至 2023-01-03
- **验证集**：2023-01-04 至 2023-12-29
- **测试集**：2024-01-02 至 2025-04-24

这种划分方式避免了数据泄露问题，确保模型在训练时只能使用历史数据，更符合实际应用场景。
...
**注**：本文档基于 `ml_model/model/train_xgboost.py` 和 `ml_model/config/xgboost.yaml` 实现代码生成，详细的技术实现细节可参考源代码。
