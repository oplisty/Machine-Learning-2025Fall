...
def plot_loss_curve(
    ...
    plt.title(f"LSTM 训练 vs 验证 {metric_name.upper()}")
...
def save_results(
    ...
    for col in ["close", "volume"]:
        if col in target_cols:
            ...
            plt.title(f"LSTM {col} 预测结果 ({prefix})")
            plt.xlabel("时间")
            plt.ylabel(col)
            plt.legend()
            plt.grid(True, alpha=0.3)
...
def save_metrics_log(
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        f.write("LSTM 模型评估指标 (RMSE/MAE 越小越好, R2 越大越好):\n")
        f.write("=" * 60 + "\n\n")
...
@hydra.main(version_base=None, config_path="../config", config_name="lstm")
def main(cfg: DictConfig):
    ...
    checkpoints_output_dir_cfg = cfg.get("checkpoints_output_dir", "ml_model/checkpoints/lstm")
    ...
    model, train_metrics, valid_metrics, y_train_pred, y_valid_pred, train_losses, valid_losses = train_lstm_model(
