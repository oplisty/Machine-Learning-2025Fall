...
def load_xgb_results_and_features():
    ...
    if not (os.path.exists(train_path) and os.path.exists(valid_path) and os.path.exists(test_path)):
        raise FileNotFoundError("找不到 XGBoost train/valid/test 结果，请检查 ml_model/output/xgboost/horizon_5 目录。")
...
class Conviction_Filter_Strategy(BaseMLStrategy):
    """
    仓位 ∈ {0.0, 0.5, 1.0}
